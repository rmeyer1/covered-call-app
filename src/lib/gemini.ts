...
const GEMINI_MODEL =
  process.env.GEMINI_MODEL || process.env.GOOGLE_GEMINI_MODEL || DEFAULT_GEMINI_MODEL;
...
function buildHoldingsPrompt(ocrText?: string) {
  ...
  const base = [
    ...
    '- marketValue is total position value when explicitly shown.',
    '- costBasis should be the average cost / cost basis per share when explicitly shown.',
    '- Do NOT infer costBasis from marketValue or shares. Use null if missing.',
...
export async function analyzeHoldingsWithGemini({
  ...
  try {
    const { data } = await axios.post(GEMINI_ENDPOINT, payload, {
