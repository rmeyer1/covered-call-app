...
import type { LongCallSuggestion } from '@/types';
...
export default function LongCallsTable({ currentPrice, suggestions }: Props) {
              ...
              <th className="px-4 sm:px-6 py-2 sm:py-3 whitespace-nowrap relative align-middle">
                ...
                {showDeltaTip && (
                  <div className="absolute z-30 left-auto right-0 sm:left-0 sm:right-auto top-full mt-2 max-w-[14rem] sm:max-w-[16rem] max-h-40 overflow-auto p-2 text-xs font-medium text-white bg-gray-900 rounded shadow-lg dark:bg-gray-700 break-words whitespace-normal">
                    Sensitivity to a $1 stock move; ~probability of finishing ITM. Higher = more stock-like.
                  </div>
                )}
              </th>
              <th className="px-4 sm:px-6 py-2 sm:py-3 whitespace-nowrap relative align-middle">
                ...
                {showThetaTip && (
                  <div className="absolute z-30 left-auto right-0 sm:left-0 sm:right-auto top-full mt-2 max-w-[14rem] sm:max-w-[16rem] max-h-40 overflow-auto p-2 text-xs font-medium text-white bg-gray-900 rounded shadow-lg dark:bg-gray-700 break-words whitespace-normal">
                    Time decay per day. Negative for long calls; larger magnitude = faster decay.
                  </div>
                )}
              </th>
              <th className="px-4 sm:px-6 py-2 sm:py-3 whitespace-nowrap relative align-middle">
                ...
                {showIvTip && (
                  <div className="absolute z-30 left-auto right-0 sm:left-0 sm:right-auto top-full mt-2 max-w-[14rem] sm:max-w-[16rem] max-h-40 overflow-auto p-2 text-xs font-medium text-white bg-gray-900 rounded shadow-lg dark:bg-gray-700 break-words whitespace-normal">
                    Implied volatility. Higher IV inflates premiums; long calls benefit if IV rises.
                  </div>
                )}
              </th>
              <th className="px-4 sm:px-6 py-2 sm:py-3 whitespace-nowrap relative align-middle">
                ...
                {showIntrinsicTip && (
                  <div className="absolute z-30 left-auto right-0 sm:left-0 sm:right-auto top-full mt-2 max-w-[14rem] sm:max-w-[16rem] max-h-40 overflow-auto p-2 text-xs font-medium text-white bg-gray-900 rounded shadow-lg dark:bg-gray-700 break-words whitespace-normal">
                    Immediate value if exercised: max(spot − strike, 0).
                  </div>
                )}
              </th>
              <th className="px-4 sm:px-6 py-2 sm:py-3 whitespace-nowrap relative align-middle">
